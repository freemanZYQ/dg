                                                 ...
                                                 const llvm::Function *F)
{
    ...
    callNode->addSuccessor(subg.getRoot());

    // update callgraph
    auto parentEntry = subgraphs_map[CInst->getParent()->getParent()].getRoot();
    assert(parentEntry);
    PS.registerCall(parentEntry, subg.getRoot());
    ...
    if (subg.getRoot()) {
        ...
        subg.getRet()->addSuccessor(returnNode);
    } else {
...
LLVMPointerGraphBuilder::buildFunction(const llvm::Function& F)
{
    ...
    s.subgraph = PS.createSubgraph(F.getName().str());
    s.subgraph->setRoot(root);
    s.subgraph->setVararg(root);
    ...
    assert(subgraphs_map[&F].getRoot() != nullptr);
    return s;
}
                                                            ...
                                                            PSNode *callNode)
{
    ...
    if (F->isVarArg()) {
        ...
        if (CI)
            // funcptr call
            addVariadicArgumentOperands(F, CI, subg.getVararg());
        else
            addVariadicArgumentOperands(F, subg.getVararg());
    }

    if (subg.getRet()) {
        addReturnNodeOperands(F, subg.getRet(), callNode);
    }
}
...
PointerGraph *LLVMPointerGraphBuilder::buildLLVMPointerGraph()
{
    ...
    assert(subg.subgraph);
    PS.setEntry(subg.subgraph);
